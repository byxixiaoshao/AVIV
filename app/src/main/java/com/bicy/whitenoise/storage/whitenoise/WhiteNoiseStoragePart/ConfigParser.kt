...
object ConfigParser {
    ...
    fun parseCreativeEffectConfig(json: JSONObject?): CreativeEffectConfig {
        ...
        return CreativeEffectConfig(
            ...
            hifi = json.optDouble("hifi", 0.0).toFloat(),
            stereoWidener = json.optDouble("stereoWidener", 0.5).toFloat(),
            virtualBass = json.optDouble("virtualBass", 0.2).toFloat(),
            multibandCompressor = json.optDouble("multibandCompressor", 0.5).toFloat()
        )
    }
    ...
    fun toJson(config: CreativeEffectConfig): JSONObject {
        return JSONObject().apply {
            ...
            put("hifi", config.hifi)
            put("stereoWidener", config.stereoWidener)
            put("virtualBass", config.virtualBass)
            put("multibandCompressor", config.multibandCompressor)
        }
    }
