        ...
        for (int stage : config_.effectOrder) {
            switch (stage) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    // AutoEQ: analyze spectrum and adjust EQ gains in real-time
                    if (config_.eqEnabled && autoEqEngine_.isEnabled()) {
                        autoEqEngine_.process(decodeBuffer_.data(), chunkFrames, channelCount_);
                        auto* eq = effectManager_->getEqualizer();
                        if (eq) {
                            const auto& gains = autoEqEngine_.getCurrentGains();
                            for (int b = 0; b < audiofx::AUTO_EQ_BANDS; b++) {
                                eq->setParameter(b, gains[b]);
                            }
                        }
                    }
...
void AudioTrack::setAutoEqTargetCurve(const std::string& type) {
    // Deprecated: static compensation mode does not use target curve
}

void AudioTrack::setAutoEqIntensity(float intensity) {
    autoEqEngine_.setIntensity(intensity);
    // Apply gains to EQ immediately
    auto* eq = effectManager_->getEqualizer();
    if (eq) {
        const auto& gains = autoEqEngine_.getCurrentGains();
        for (int b = 0; b < audiofx::AUTO_EQ_BANDS; b++) {
...
void AudioTrack::setAutoEqResponseSpeed(const std::string& speed) {
    // Deprecated: static compensation mode does not use response speed
}

void AudioTrack::setAutoEqMaxBoost(float db) {
    // Deprecated: static compensation mode does not use max boost
}

void AudioTrack::setAutoEqMaxCut(float db) {
    // Deprecated: static compensation mode does not use max cut
}

void AudioTrack::setAutoEqSmoothing(float s) {
    // Deprecated: static compensation mode does not use smoothing
}
...
void AudioTrack::setAutoEqDynamicQEnabled(bool enabled) {
    // Deprecated: static compensation mode does not use dynamic Q
}

void AudioTrack::setAutoEqAttack(float ms) {
    // Deprecated: static compensation mode does not use attack
}

void AudioTrack::setAutoEqRelease(float ms) {
    // Deprecated: static compensation mode does not use release
}

void AudioTrack::setAutoEqMaxSlope(float slope) {
    // Deprecated: static compensation mode does not use max slope
}

void AudioTrack::setAutoEqCouplingCoeff(float coeff) {
    // Deprecated: static compensation mode does not use coupling
}

void AudioTrack::setAutoEqHysteresis(float db) {
    // Deprecated: static compensation mode does not use hysteresis
}
...
std::array<float, audiofx::AUTO_EQ_BANDS> AudioTrack::getAutoEqGains() const {
