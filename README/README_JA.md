...
**リアルタイム C++ オーディオエンジンを中心とした Android ホワイトノイズ & 音楽プレイヤー。十数種類のプロフェッショナルエフェクト、空間オーディオ、AutoEQ、オンラインストリーミングを統合。**
...
### 🌊 ホワイトノイズ
- カテゴリ別に整理された複数の内蔵音源、ローカル/オンライン対応
...
- プレイリスト管理
- **QuickJS 音源スクリプトエンジン**によるオンラインストリーミング、マルチエンジン自動検出 & タイムアウト時の自動フェイルオーバー
...
> ビルドには `keystore.properties` 署名設定が必要です。SoundTouch ソースはプロジェクトルートの `soundtouch/` にあり、CMake が自動的に静的ライブラリとしてコンパイル・リンクします。
