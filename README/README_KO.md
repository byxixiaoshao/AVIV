...
**실시간 C++ 오디오 엔진을 중심으로 구축된 Android 화이트노이즈 및 음악 플레이어. 10여 종의 전문 이펙트, 공간 오디오, AutoEQ를 통합.**
...
### 🌊 화이트노이즈
- 카테고리별로 정리된 다수의 내장 음원, 로컬 음악 재생 지원
...
> 빌드에는 `keystore.properties` 서명 설정이 필요합니다. SoundTouch 소스는 직접 구하셔야 합니다.
